...
def nms_temporal(predict_score, predict_windows, overlap):
    ...
    unions = ends - starts
    indices = [x[0] for x in sorted(enumerate(scores), key=lambda x:x[1])] # sort and get index

    while len(indices)>0:
        i = indices[-1]  # max score
        pick.append(i)

        lefts = [max(starts[i],starts[j]) for j in indices[:-1]]
        rights = [min(ends[i],ends[j]) for j in indices[:-1]]
        inters = [max(0.0, right-left) for left, right in zip(lefts, rights)]
        laps = [inters[u]/(unions[i] + unions[indices[u]] - inters[u]) for u in range(len(indices)-1)]
        indices_new = []
